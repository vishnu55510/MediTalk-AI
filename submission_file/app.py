...
health_data_collector = ChatCompletionAgent(
    ...
    instructions="Collect user health data and store it in MySQL and Pinecone.",
...
mysql_query_agent = ChatCompletionAgent(
    ...
    instructions="Fetch patient data from MySQL health database table name `health_info`.",
...
hospital_search_agent = ChatCompletionAgent(
    ...
    plugins=[serp_api_location_search_plugin]
)
...
async def on_message(message: cl.Message):
    ...
    await cl.Message(content=f"🩺 SmartHealth AI: {triage_response.message.content}").send()
